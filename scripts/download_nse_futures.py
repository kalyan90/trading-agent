"""Download official NSE NIFTY futures contract history by calendar year."""
import argparse
import csv
...
def main():
    ...
    for year in range(args.start_year, args.end_year + 1):
        rows = client.fetch_futures_history(date(year, 1, 1), min(date(year, 12, 31), date.today()))
        rows.sort(key=lambda row: (datetime.strptime(row["date"], "%d-%b-%Y"), datetime.strptime(row["expiry"], "%d-%b-%Y")))
        path = args.output_dir / f"nifty_futures_contracts_{year}.csv"
