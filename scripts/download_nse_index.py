"""Download official NSE index history by calendar year."""
import argparse
import csv
...
def main():
    ...
    for year in range(args.start_year, args.end_year + 1):
        rows = client.fetch_index_history(
            date(year, 1, 1), min(date(year, 12, 31), date.today()),
            index=args.index,
        )
        rows.sort(key=lambda row: datetime.strptime(row["date"], "%d-%b-%Y"))
        slug = args.index.lower().replace(" ", "_").replace("&", "and")
        path = args.output_dir / f"{slug}_index_{year}.csv"
